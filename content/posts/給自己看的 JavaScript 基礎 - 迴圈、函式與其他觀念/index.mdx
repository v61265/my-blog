...
| Math.floor(num)       | 無條件捨去                                   | `var floor = Math.floor(10.1)` //10                    |
...
只要還是陣列， .map 和 .filter 可以一起使用，也可以無限往後加。
...
上面數字的排序只是看最前面的數字，要真正幫數字排序可使用函數。概念為若回傳值小於等於 0 不調換，但若 > 0 則互換。詳細可參考[MDN](https://developer.mozilla.org/zh-TW/docs/Web/JavaScript/Reference/Global_Objects/Array/sort)因此若要由小排到大，可用：
